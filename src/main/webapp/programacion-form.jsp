...
<html>
<head>
<title>Insertar / Actualizar  Clases</title>
</head>
<body>
<header>
<div class="header">
...
<a href="javascript:void(0);" style="font-size:15px;" class="icon" onclick="myFunction()">&#9776;</a>
	...
	<div>
		<div>
			<div>
				<c:if test="${programacion != null}">
					<form action="update" method="post" class="formulario">
					<h3>EDITAR CLASE</h3>
				<fieldset>
					<legend>ID</legend>
					<input type="number" value="<c:out value='${programacion.id}' />"
						class="form-control" name="id" readonly>
				</fieldset>
				</c:if>
				<c:if test="${programacion == null}">
					<form action="insert" method="post" class="formulario">
					<h3>INSERTAR NUEVA CLASE</h3>
				</c:if>
				<fieldset>
					<legend>FECHA</legend>
					<input type="date" value="<c:out value='${programacion.fecha}' />"
						class="form-control" name="fecha" >
				</fieldset>
				<fieldset>
					<legend>COLEGIO</legend>
					<select name="id_col" id="id_col">
						<!-- for (Todo todo: todos) { -->
						<c:forEach var="colegio" items="${listColegios}">
							<option value="<c:out value="${colegio.id}" />"> ${colegio.nombre}</option>
						</c:forEach>
						<!-- } -->
					</select>
				</fieldset>
				...
				<fieldset>
					...
					<select name="id_prof" id="id_prof">
						<!-- for (Todo todo: todos) { -->
						<c:forEach var="profesor" items="${listProfesores}">
							<option value="<c:out value="${profesor.id}" />"> ${profesor.name}</option>
						</c:forEach>
						<!-- } -->
					</select>
				</fieldset>
				...
				<legend>COMENTARIOS</legend>
					<textarea class="form-control" name="coment" required="required" rows="10" cols="40" value="<c:out value='${programacion.coment}' />">
Grupo: 
Horario de clase: 
					</textarea> 
				</fieldset>
